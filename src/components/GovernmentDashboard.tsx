...
export default function GovernmentDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "installations" | "applications"
  >("overview");
  ...
  const loadData = async () => {
    try {
      ...
      if (installationsData) {
        ...
        const totalWaterSaved = installationsData.reduce(
          (sum, inst) => sum + (inst.annual_water_saved || 0),
          0
        );
        const compliantCount = installationsData.filter(
          (inst) => inst.compliance_status === "compliant"
        ).length;

        setStats({
          totalInstallations: installationsData.length,
          activeInstallations: installationsData.filter(
          ...
          complianceRate:
            installationsData.length > 0
              ? (compliantCount / installationsData.length) * 100
              : 0,
        });
      }
  ...
  const getApplicationsByMonth = () => {
    ...
    applications.forEach((app) => {
      const month = new Date(app.submitted_at).toLocaleDateString("en-US", {
        month: "short",
        year: "numeric",
      });
  ...
  const exportReport = () => {
    ...
    a.href = url;
    a.download = `rtrwh-installations-${
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Monitor and manage RTRWH installations across your jurisdiction
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
            ...
            <p className="text-3xl font-bold text-gray-900">
              {(stats.totalWaterSaved / 1000).toFixed(1)}k L
            </p>
          </div>
                      ...
                      {installations
                        ...
                        .map((installation) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-sm text-gray-900">
                              {installation.tank_capacity.toLocaleString()} L
                            </td>
                            ...
                            <td className="px-4 py-3 text-sm text-gray-900">
                              {installation.annual_water_saved?.toLocaleString() ||
                                0}{" "}
                              L/year
                            </td>
                          </tr>
